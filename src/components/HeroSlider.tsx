...
export function HeroSlider() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-navy-deep/95 via-navy-deep/45 to-navy-deep/10" />
      ...
      <div className="relative z-10 h-full flex items-center max-w-7xl mx-auto px-4 sm:px-6 md:px-12">
        <div className={`max-w-2xl transition-all duration-700 ${isTransitioning ? 'opacity-0 translate-y-6' : 'opacity-100 translate-y-0'}`}>
          <span className="inline-block text-[9px] sm:text-[10px] md:text-[11px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.3em] text-orange mb-4 sm:mb-6 border-b border-orange pb-1">
            {slide.tag}
          </span>

          <h1 className="font-display text-3xl sm:text-5xl md:text-7xl leading-[1.15] sm:leading-[1.1] mb-4 sm:mb-6 whitespace-pre-line text-orange drop-shadow-lg">
            {slide.heading}
          </h1>
          ...
          <div className="flex flex-wrap gap-3 sm:gap-4">
            <Link
              href={slide.href}
              className="inline-flex items-center gap-2 sm:gap-3 px-5 sm:px-8 py-3 sm:py-4 bg-orange text-navy-deep font-bold uppercase tracking-widest text-[11px] sm:text-sm hover:bg-white transition-all shadow-[0_0_20px_rgba(212,175,55,0.4)]"
            >
              {slide.cta}
            </Link>
            <Link
              href="#"
              className="inline-flex items-center gap-2 sm:gap-3 px-5 sm:px-8 py-3 sm:py-4 border border-orange/50 text-orange font-bold uppercase tracking-widest text-[11px] sm:text-sm hover:bg-orange/10 transition-all"
            >
              Book Consultation
            </Link>
          </div>
